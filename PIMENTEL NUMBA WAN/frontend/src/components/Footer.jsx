    ...
    <footer className="flexCenter pb-24 pt-20">
      <div className="max_padd_container flex w-full flex-col gap-14">
        <div className="flex flex-col items-start justify-center gap-[10%] md:flex-row">
          <Link to={'/'} className="mb-10 bold-20" >Shoopee</Link>
            ...
            <FooterColumn title={FOOTER_CONTACT_INFO.title}>
              {FOOTER_CONTACT_INFO.links.map((link)=>(
                <Link to={"/"} key={link.label} className="flex gap-4 md:flex-col lg:flex-row">
                  <p>{link.label}:</p><p className="meduim-14">{link.value}</p>
                </Link>
              ))}
            </FooterColumn>
          </div>
          ...
          <p className="text-center regular-14 text-gray-30">2030 XTRM | All rights reserved.</p>
      </div>
    </footer>
  )
}
